...
public class Filme : Entity
{
        ...
        int duracao, string sinopse, string urlImagem)
    {
        ...
        if (string.IsNullOrWhiteSpace(tituloOriginal) || tituloOriginal.Length < 2)
        ...
        if (string.IsNullOrWhiteSpace(sinopse) || sinopse.Length < 2)
            _errors.Add("Sinopse inválida");
        
        if (string.IsNullOrWhiteSpace(urlImagem) || Uri.TryCreate(urlImagem, UriKind.RelativeOrAbsolute, out Uri uri) is false)
